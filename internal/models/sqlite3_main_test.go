...
func (s *sqliteTester) setup() error {
	...
	if err = dumpCmd.Start(); err != nil {
		return fmt.Errorf("failed to start sqlite3 dump command: %w", err)
	}
	if err = createCmd.Start(); err != nil {
		return fmt.Errorf("failed to start sqlite3 create command: %w", err)
	}

	if err = dumpCmd.Wait(); err != nil {
		fmt.Println(err)
		return fmt.Errorf("failed to wait for sqlite3 dump command: %w", err)
	}
	...
	if err = createCmd.Wait(); err != nil {
		fmt.Println(err)
		return fmt.Errorf("failed to wait for sqlite3 create command: %w", err)
	}
