...
func testTasksInsertWhitelist(t *testing.T) {
	...
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	if err = o.Insert(ctx, tx, boil.Whitelist(strmangle.SetMerge(taskPrimaryKeyColumns, taskColumnsWithoutDefault)...)); err != nil {
		t.Error(err)
	}
...
var (
	taskDBTypes = map[string]string{`ID`: `INTEGER`, `Title`: `TEXT`, `Done`: `BOOLEAN`, `UserID`: `INTEGER`}
	_           = bytes.MinRead
)
...
func testTasksSliceUpdateAll(t *testing.T) {
	...
	if strmangle.StringSliceMatch(taskAllColumns, taskPrimaryKeyColumns) {
		...
		fields = strmangle.SetComplement(fields, taskGeneratedColumns)
	}
